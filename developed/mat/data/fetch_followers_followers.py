...
layer2_out = 'followers_followers.csv'
...
def appendFollowers(id, i, total_i, total_count):
	count = 0
	with open(layer2_out,'a') as f:
		followerIds = tweepy.Cursor(api.followers_ids,id).items()
		while True:
			try:
				followerId = followerIds.next()
				print("%d/%d\t %d/%d" % (i, total_i, count, total_count))
				f.write("%s,%s\n" % (str(id),str(followerId)))
				count += 1
...
def showTimeRequired(layer1):
	...
	for i in range(len(layer1)):
		minutes_required += layer1[i][1] / 5000 + 1
	print("# followers above threshold: %d" % (len(layer1)))
...
for i in range(len(layer1)):
	appendFollowers(layer1[i][0],i,len(layer1),layer1[i][1])
